...
Future<http.Response> getCyberExpertDashboardCases() async {
  ...
  Future<http.Response> getCaseEvidence(dynamic caseId) async {
    final prefs = await SharedPreferences.getInstance();
    final accessToken = prefs.getString("access_token");
  ...
  // GET Single Evidence Hash Details
  ...
  // POST Upload Evidence to Case
  Future<http.StreamedResponse> uploadCaseEvidence(
    dynamic caseId,
    List<int> fileBytes,
    String fileName, {
    String? originalHash,
    ...
    final request = http.MultipartRequest(
      'POST',
      Uri.parse(ApiConstants.uploadCaseEvidence(caseId)),
    );
    ...
    if (originalHash != null && originalHash.trim().isNotEmpty) {
      request.fields['original_hash'] = originalHash.trim();
    }
