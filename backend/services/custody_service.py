...
class CustodyService:
    ...
    def get_or_create_current_custody(db: Session, evidence_id: str, case_id: Optional[str] = None) -> CurrentCustodyInfo:
        ...
        if not info:
            effective_case_id = CustodyService._resolve_case_id(db, clean_ev_id, case_id) or (str(case_id) if case_id else "UNKNOWN")
            ...
            if case_id and (not info.case_id or info.case_id == "UNKNOWN"):
                info.case_id = str(case_id)
                db.commit()
                db.refresh(info)
        return info
    ...
    def get_custody_summary(db: Session, evidence_id: str) -> Dict[str, Any]:
        """
        Evidence-specific summary cards:
        - Total Events
        ...
        return {
            ...
            "transfers_count": completed_transfers,
    ...
    @staticmethod
    def get_case_custody_summary(db: Session, case_id: str) -> Dict[str, Any]:
        """
        Case-level summary metrics dynamically derived from stored records:
        - Total Events across all case evidence
        - Handlers (distinct human handlers)
        - Transfers (completed)
        - Pending Transfers
        - First Handled timestamp
        ...
        completed_transfers = db.query(TransferRecord).filter(
            TransferRecord.case_id == clean_case_id,
            TransferRecord.status == "COMPLETED"
        ).count()
        ...
        # Derive case-level custody status from latest active evidence state
        latest_custody = db.query(CurrentCustodyInfo).filter(
            CurrentCustodyInfo.case_id == clean_case_id
        ).order_by(CurrentCustodyInfo.updated_at.desc()).first()

        current_status = latest_custody.custody_status if latest_custody else None
    ...
    def get_custody_timeline(
        ...
        """
        Chronological custody timeline with event filtering.
        Supports filtering by evidence_id and/or case_id.
        """
        query = db.query(CustodyLog)

        if evidence_id:
            query = query.filter(CustodyLog.evidence_id == str(evidence_id).strip())
        if case_id:
            query = query.filter(CustodyLog.case_id == str(case_id).strip())
        ...
        for l in logs:
            timeline_items.append({
                ...
                "transfer_sender": l.transfer_sender,
                "transfer_recipient": l.transfer_recipient,
    ...
    def update_current_custody(
        ...
        if changes:
            ...
            c_log = CustodyLog(
                ...
                actor_role=actor_role or "Cyber Expert",
    ...
    def initiate_transfer(
        ...
        if case_id and info.case_id and str(info.case_id) != str(case_id):
        ...
        custody_entry = CustodyLog(
            ...
            actor_role=sender_role or "Cyber Expert",
    ...
    def receive_transfer(
        ...
        Updates current holder, assigned timestamp, and marks status as 'In Analysis'.
