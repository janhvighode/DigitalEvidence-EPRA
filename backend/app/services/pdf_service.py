...
class PDFService:
    ...
    def generate_pdf(
        ...
        case_id = report_data.get("case_id", "UNKNOWN_CASE")
        ...
        if "metadata_summary" in selected_sections:
            ...
            story.append(Paragraph("1. Metadata & Verification Summary", h2_style))
        ...
        if "chain_of_custody" in selected_sections:
            ...
            if not c_logs:
                ...
                for c in c_logs:
                    ...
                    actor = f"{c.get('actor_name') or c.get('investigator_name', 'Unknown')}"
                    if c.get("actor_role"):
                        actor += f" ({c['actor_role']})"
                    t_stamp = c.get("timestamp", "N/A")
                    if t_stamp and len(t_stamp) > 16:
                        t_stamp = t_stamp[:16].replace("T", " ")
                    res = c.get("outcome") or c.get("result", "SUCCESS")
                    rem = c.get("description") or c.get("remarks", "")
                    ...
                    c_rows.append([
                        ...
                        Paragraph(cls._safe_text(t_stamp), body_style),
                        Paragraph(cls._safe_text(res), body_bold),
                        Paragraph(cls._safe_text(rem), body_style)
                    ])

                c_table = Table(c_rows, colWidths=[35, 35, 110, 110, 75, 55, 120])
        ...
        if "suspect_summary" in selected_sections:
            ...
            if not suspects:
                ...
                for s in suspects:
                    ...
                    role = s.get("role_or_relation", "Person of Interest")
                    rank = str(s.get("externally_supplied_ranking") or "Unranked")
        ...
        if "epra_analysis" in selected_sections:
            ...
            if not epra_results:
                ...
                for res in epra_results:
                    ...
                    rank = str(res.get("priority_rank") or "N/A")
                    score = f"{res.get('priority_score', 0):.2f}" if res.get("priority_score") is not None else "N/A"
                    status = res.get("status", "PENDING")
                    prov = res.get("provenance_source", "EPRA Model")
                    notes = res.get("notes") or res.get("missing_input_reason") or "None"
        ...
        if "activity_logs" in selected_sections:
            ...
            if not act_logs:
                ...
                a_table = Table(a_rows, colWidths=[95, 125, 55, 75, 190])
        ...
        if "conclusions" in selected_sections:
            ...
            if not conclusions and not recommendations:
                ...
                if conclusions:
                    story.append(Paragraph(f"<b>Forensic Findings & Conclusions:</b>", body_bold))
                    story.append(Paragraph(cls._safe_text(conclusions), body_style))
                    story.append(Spacer(1, 6))
                if recommendations:
                    story.append(Paragraph(f"<b>Investigative Recommendations:</b>", body_bold))
